...
const ViewTeacher = () => {
    ...
    const { data: teachers, refetch } = useQuery(['teachers', clickedPage], () => fetch(`https://tsc-server.onrender.com/teachers?page=${clickedPage}`).then(res => res.json()));

    useEffect(() => {
        const getPageNumbers = async () => {
            const request = await fetch('https://tsc-server.onrender.com/teachersCount');
